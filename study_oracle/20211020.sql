...
컬럼표현 LIKE % + 검색문자 + %
성명 LIKE '홍%'  ☞ 성명이 홍으로 시작하는 모든것 : 홍길동, 홍시, 홍
성명 LIKE '%홍'  ☞ 성명이 홍으로   끝나는 모든것 : 분홍, 다홍, 홍
성명 LIKE '%홍%' ☞ 성명이 홍을   포함하는 모든것 : 홍길동, 홍시, 홍, 분홍, 다홍
...
05. 성명에 대소문자 무관하게 z 가 포함된 성을 가진 사원들의
--  ￣￣
사번, 이름, 성, 입사일자 조회
...
06. 성에 소문자 z 가 앞에서 2번째에 위치해 있는 성을 가진 사원들의
사번, 이름, 성, 입사일자 조회
SELECT  employee_id, first_name, last_name, hire_date --1
FROM    employees
WHERE   last_name LIKE '_z%';

07. 성에 소문자 z 가 앞에서 5번째 위치해있는 성을 가진 사원들의
사번, 이름, 성, 입사일자 조회
SELECT  employee_id, first_name, last_name, hire_date --2
FROM    employees
WHERE   last_name LIKE '____z%';

08. 성에 소문자 z가 뒤에서부터 5번째 위치해 있는 성을 가진 사원들의
사번, 이름, 성, 입사일자 조회
SELECT  employee_id, first_name, last_name, hire_date --1
FROM    employees
WHERE   last_name LIKE '%z____';

LIKE 연산자 역시 BETWEEN 이나 IN 과 같이 NOT 연산자를 함께 사용할 수 있다.
09. 전화번호가 6으로 시작되지 않은 사원의
사번, 이름, 성, 입사일자 조회
SELECT  employee_id, first_name, last_name, hire_date --61
FROM    employees
...
FROM    employees
--WHERE   job_id LIKE '%_A%'; --XXX, A앞에 한글자 있는 모든것
WHERE   job_id LIKE '%\_A%' ESCAPE '\';

8. 데이터 값이 없는 형태의 표현 : NULL -- 비교불가, 연산불가
...
정렬기준표현 : 컬럼명, ALIAS 명, SELECT 목록에서의 컬럼위치번호
프로그램상에서는 정렬 잘 안함(즉, DB에서만 정렬해서 프로그램으로 넘김)

01. 80번 부서의 사번, 성, 급여, 부서코드에 대해 성을 오름차순/내림차순으로 정렬
...
--ORDER BY 2 ASC;         --컬럼위치
ORDER BY 성 ASC;       --ALIAS

02. 60번 부서의 사번, 성, 연봉 에 대해 연봉을 기준으로 오름차순
SELECT  employee_id, last_name, salary * 12 연봉
FROM    employees
WHERE   department_id = 60
--ORDER BY 연봉; --ALIAS
--ORDER BY 3; --컬럼목록 위치 번호
...
1) 반올림 함수 : ROUND(숫자, 소수이하/소수이상자리수)
소수이하/소수이상 자리수 : 음수도 지정가능, 생략시 기본값은 0(정수로 표현)
SELECT  ROUND(1234.5678, 2)   r1, --1234.57
        ROUND(1234.5678, 1)   r2, --1234.6
        ROUND(1234.5678, 0)   r3, --1235, 0 : 소수이하 없음
        ROUND(1234.5678, -1)  r4, --1230, -1 : 0이 1개
        ROUND(1234.5678, -2)  r5  --1200  -2 : 0이 2개
FROM    dual;

2) 무조건 버림함수 : TRUNC(숫자, 소수이하/소수이상자리수)
소수이하/소수이상 자리수 : 음수도 지정가능, 생략시 기본값은 0(정수로 표현)
SELECT  TRUNC(1234.5678, 2)   t1, --1234.56
        TRUNC(1234.5678, 1)   t2, --1234.5
        TRUNC(1234.5678, 0)   t3, --1234
        TRUNC(1234.5678, -1)  t4, --1230
        TRUNC(1234.5678, -2)  t5  --1200
FROM    dual;

게시판에서 페이지 나눌때(페이징 처리)사용 : 게시글 수 / 10

3) 숫자보다 같거나 큰 정수를 반환하는 함수 : CEIL(n) : 무조건 올림 함수
SELECT  CEIL(0.999999999999999)           c1, --1
        CEIL(0.0000000000000000000001)    c2, --1
        CEIL(0)                           c3  --0
FROM    dual;


4) 숫자보다 같거나 작은 정수를 반환하는 함수 : FLOOR(n) : 무조건 내림 함수
SELECT  FLOOR(0.999999999999999)          f1, --0
        FLOOR(0.0000000000000000000001)   f2, --0
        FLOOR(0)                          f3  --0

숫자 데이터를 표현할 수 있는 함수 : ROUND, TRUNC, CEIL, FLOOR
소숫점 데이터를 표현할 수 있는 함수 : ROUND, TRUNC
정수 데이터를 표현할 수 있는 함수 : ROUND, TRUNC(두번째 파라미터가 0또는 생략), CEIL, FLOOR
