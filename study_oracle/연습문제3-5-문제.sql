...
SELECT  employee_id, last_name, department_id, salary, 
        ...
        END bonus1,
        CASE
          WHEN department_id < 10   THEN  salary * 0 --컴마없음
          WHEN department_id <= 30  THEN  salary * 0.1 --컴마없음
          WHEN department_id <= 60  THEN  salary * 0.2 --컴마없음
          WHEN department_id <= 100 THEN  salary * 0.3 --컴마없음
          ELSE salary * 0.05
        END bonus2
FROM    employees;
...
SELECT  employee_id, first_name, last_name, department_id, salary, job_id,
        CASE
          WHEN  department_id <= 20 THEN  salary * 0.3 --컴마없음
          WHEN  salary >= 10000 THEN  salary * 0.2 --컴마없음
          WHEN  LOWER(job_id) LIKE '%clerk%' THEN  salary * 0.3 --컴마없음
          ELSE  salary * 0.05
        END bonus
FROM    employees;
