...
2.1 데이터 조회
--DESC(DESCRIBE) : 테이블의 컬럼이름, 데이터형, 길이, NULL 허용여부 등 테이블의 특정 정보를 제공
...
SELECT  ☞ 테이블에 저장된 데이터를 조회하기 위한 명령어, SQL 문 중에 가장 많이 사용
          SELECT 문에는 FROM 키워드가 반드시 따라와야 함.
WHERE   ☞ SELECT 문에서 마지막에 쓸 수 있는데 원하는 레코드만 검색하고자 할때 사용
...
SELECT  *
FROM    employees; --실행 : ctrl + enter, F5
