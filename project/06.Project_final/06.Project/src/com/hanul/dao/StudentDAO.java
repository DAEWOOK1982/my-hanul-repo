...
public class StudentDAO {
	...
	//합격 불합격 입력
		public void passSucc(StudentDTO dtos) {
			SqlSession session = sqlMapper.openSession();
			session.update("com.hanul.mybatis.testMapper.xml.passSucc", dtos);
			session.commit();
			session.close();
		}
		public void passFail(StudentDTO dtos) {
			SqlSession session = sqlMapper.openSession();
			session.update("com.hanul.mybatis.testMapper.xml.passFail", dtos);
			session.commit();
			session.close();
		}
}//class
